...
st.title("📊 Indicador Markov-Queue BTC - Versão Premium")
...
def carregar_dados():
    try:
        ...
        df = pd.DataFrame({
            ...
            'Volume': dados['Volume'].values if 'Volume' in dados else np.nan
        }, index=dados.index)
...
if not dados_btc.empty:
    ...
    # Definir estados com condições aprimoradas
    condicoes = [
...
Desenvolvido para operações de médio/longo prazo com Bitcoin.
""")
